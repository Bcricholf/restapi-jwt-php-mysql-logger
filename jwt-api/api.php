<?php 
	...
	class Api extends Rest {
		...
		public function writeLog() {
			...
			$level = $this->validateParameter('level', $this->param['level'], STRING);
			$messageAdd = Logger::writeLog($message, $level);
			
			if($messageAdd === FALSE) {
				$messageReturn = 'Failed to add log.';
			} else {
				$messageReturn = "Log add.";
			}

			$this->returnResponse(SUCCESS_RESPONSE, $messageReturn);
		}

		public function generateCallTrace() {
			$level = $this->validateParameter('level', $this->param['level'], STRING);
			$giveAllHistory = $this->validateParameter('giveAllHistory', $this->param['giveAllHistory'], BOOLEAN);
			$messageAdd = Logger::generateCallTrace($level, $giveAllHistory);
			
			if($messageAdd === FALSE) {
				$messageReturn = 'Failed to generate call trace.';
			} else {
				$messageReturn = "Call trace generate and add.";
			}
